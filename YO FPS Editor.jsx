...
function checkForUpdates(Status_update, check_updates) {
    // Текущая версия (ваша локальная)
    var currentVersion = "2.8";  // <-- замените на актуальную версию вашего скрипта

    // Адрес GitHub API для последнего релиза
    var githubApiUrl = "https://api.github.com/repos/yo-romlogunov/YO-FPS-Editor/releases/latest";

    try {
        // Добавляем заголовок "User-Agent", чтобы избежать ограничений GitHub
        var curlCmd = 'curl -s -H "User-Agent: YO-FPS-Editor" "' + githubApiUrl + '"';
        var response = system.callSystem(curlCmd);

        if (response) {
            // Ищем "tag_name" и "html_url" с помощью регулярных выражений
            ...
            if (tagNameMatch && htmlUrlMatch) {
                ...
                if (compareVersions(latestVersion, currentVersion) > 0) {
                    ...
                    // Изменяем обработчик кнопки на загрузку
                    check_updates.onClick = function() {
                        openURL(downloadUrl);
                    };
                } else {
                    // Версия актуальна
                    Status_update.text = "Latest Version";
                    // Опционально можно сбросить текст кнопки, если она была изменена ранее
...
/* 
 * ---------------------------------------
 * (4) buildUI: Основная функция построения интерфейса
 * ---------------------------------------
 */
function buildUI(thisObj) {
    ...
    var pal = (thisObj instanceof Panel)
        ? thisObj
        : new Window("palette", "YO FPS Editor 2.8", undefined, { resizeable: false });

    pal.text = "YO FPS Editor 2.8";
    ...
    pal.preferredSize = [480, 600]; 
    pal.minimumSize = pal.preferredSize; 
    pal.maximumSize = pal.preferredSize; 
    ...
    // 1) Картинка (с пустым IMAGE1_IMGSTRING)
    ...
    var about_button = About_group.add("button", undefined, undefined, {name: "about_button"}); 
        ...
        about_button.helpTip = "Learn more about FPS Editor.";
    ...
    function applyChanges() {
        ...
        var isFrames = radiobutton_frames.value; 
        var durationToAdd = 0;
        ...
        var processComps = rbtn_compositions.value; // true => CompItem, false => FootageItem
        var itemsToChange = [];

        // Функции для получения/установки FPS/Duration
        ...
        function setItemFPS(it, fps) {
            if (it instanceof CompItem) {
                ...
                // Добавляем принудительную перезагрузку
                try {
                    it.mainSource.reload();
                } catch (e) {
                    // Если reload не поддерживается, можно просто перехватить ошибку
                }
            }
        }
        ...
        function setItemDuration(it, newValue) {
            if (it instanceof CompItem) {
                ...
                // Только для имидж-секвенций (isSequence = true) можно менять duration
    ...
    about_button.onClick = function() {
        ...
        // Логотип (с пустым LOGO_IMGSTRING)
        ...
        // Кнопка "Check For Updates" (с пустым CHECK_UPDATES_IMGSTRING)
        ...
        // Текстовое поле статуса обновлений
        ...
        // Привязываем проверку обновлений при нажатии на кнопку
        check_updates.onClick = function() {
            checkForUpdates(Status_update, check_updates);
        };
        ...
        // Показываем диалоговое окно About
        dialog.show();
    };

    // Обновляем UI при запуске
    updateUI();

    // Возвращаем панель (или окно)
    return pal;
}

/* 
 * ---------------------------------------
 * Запуск скрипта (панель/окно)
 * ---------------------------------------
 */
